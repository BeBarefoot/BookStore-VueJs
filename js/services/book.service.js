...
function deleteFeedback(bookId, feedbackIdx) {
    var book = booksDB.find(book => book.id === bookId);
    book.reviews.splice(feedbackIdx, 1);
    storageService.store(BOOKS_KEY, booksDB);
}

function checkIfReview(bookId) {
    let book = getBookById(bookId);
    return storageService.load(book);
}
